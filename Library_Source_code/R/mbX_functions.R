...
ezstat <- function(microbiome_data, metadata, level, selected_metadata) {
  ...
  report_file <- file.path(dir_name, "CLD_data_quality_report.txt")
  report_lines <- c(
    "CLD Data Quality Report",
    paste0("Citation: mbX (doi:10.3390/stats8020044)"),
    ""
  )
  for (tax in sig_taxa) {
    tmp <- df_stat %>%
      group_by(!!sym(selected_metadata)) %>%
      summarize(n_non_na = sum(!is.na(.data[[tax]])), .groups = "drop")
    if (all(tmp$n_non_na >= 2)) {
      report_lines <- c(report_lines,
                        paste0(tax, ": has at least two non-NA observations per group"))
    } else {
      report_lines <- c(report_lines,
                        paste0(tax, ": does NOT have at least two non-NA observations per group"))
    }
  }
  writeLines(report_lines, report_file)
  message("→ data-quality report written to: ", report_file)
  # — end insertion —
